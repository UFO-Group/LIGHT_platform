...
def extract_runs_from_file(filepath):
...
def extract_formula_arguments(run_content, formula_num):
    """
...
def extract_from_rejected_section(run_content, formula_num):
...
def extract_from_rationale_tables(run_content, formula_num):
...
def extract_comparative_arguments(run_content, formula_num):
...
def process_model(model_name, filepath):
    ...
    try:
        runs = extract_runs_from_file(filepath)

        for run_num, run_content in runs:
            # Extract arguments against Formula 4
            args_4 = extract_formula_arguments(run_content, 4)
            args_4.extend(extract_from_rejected_section(run_content, 4))
            args_4.extend(extract_from_rationale_tables(run_content, 4))
            args_4.extend(extract_comparative_arguments(run_content, 4))

            for arg in args_4:
                all_args_4.append(f"{model_name} Run {run_num}: {arg}")

            # Extract arguments against Formula 5
            args_5 = extract_formula_arguments(run_content, 5)
            args_5.extend(extract_from_rejected_section(run_content, 5))
            args_5.extend(extract_from_rationale_tables(run_content, 5))
            args_5.extend(extract_comparative_arguments(run_content, 5))
...
def deduplicate_arguments(arguments):
...
def write_anti_formula_file(formula_num, arguments, output_dir="."):
    """Write the collected arguments to a markdown file."""
    script_dir = os.path.dirname(os.path.abspath(__file__))
    results_dir = os.path.join(os.path.dirname(script_dir), "results")
    os.makedirs(results_dir, exist_ok=True)
    filename = os.path.join(results_dir, f"anti-Formula {formula_num}.md")
...
def main():
    ...
    for model_name, filename in MODEL_FILES.items():
        ...
        args_4, args_5 = process_model(model_name, filename)
        all_arguments_4.extend(args_4)
        all_arguments_5.extend(args_5)

    # Deduplicate
    print("\nDeduplicating arguments...")
    unique_args_4 = deduplicate_arguments(all_arguments_4)
    unique_args_5 = deduplicate_arguments(all_arguments_5)

    # Write output files
    print("\nWriting output files...")
    write_anti_formula_file(4, unique_args_4)
    write_anti_formula_file(5, unique_args_5)

    print("\n" + "=" * 60)
    print(f"Extraction complete!")
    print(f"  anti-Formula 4.md: {len(unique_args_4)} unique arguments")
    print(f"  anti-Formula 5.md: {len(unique_args_5)} unique arguments")
