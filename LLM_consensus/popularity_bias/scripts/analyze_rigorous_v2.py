...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(
            os.path.join(".", "rigorous_analysis_v2.log")
        ),
    ],
)
...
class RigorousBiasAnalyzer:
    ...
    def __init__(self, base_dir: str = "."):
        ...
        self.output_dir = os.path.join(os.path.dirname(base_dir), "results")
    ...
    def _load_formula_materials(self) -> Dict:
        with open(os.path.join(self.data_dir, "formula_materials.json"), "r", encoding="utf-8") as f:
            return json.load(f)

    def _load_extracted_data(self) -> Dict:
        with open(os.path.join(self.data_dir, "extracted_data.json"), "r", encoding="utf-8") as f:
            return json.load(f)
...
def main():
    ...
    analyzer = RigorousBiasAnalyzer(base_dir=script_dir)
